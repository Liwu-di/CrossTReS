...
class KinematicObservation(ObservationType):
    ...
    FEATURES: List[str] = ["presence", "x", "y", "vx", "vy", "cos_h", "sin_h", "pre_x", "pre_y"]
    # FEATURES: List[str] = ["presence", "x", "y", "vx", "vy"]
    INTENTION_FEATURES: List[str] = ["presence", "x", "y", "vx", "vy", "cos_h", "sin_h"]
    # ALL_FEATURES: List[str] = ["presence", "x", "y", "vx", "vy", "heading", "cos_h", "sin_h", "cos_d", "sin_d"]
    ...
    def __init__(self, env: 'AbstractEnv',
        ...
        self.intention_feature = self.INTENTION_FEATURES
    ...
    def pre_intention(self, obsfornn: np.ndarray) -> np.ndarray:
        a0 = obsfornn[0]
        a1 = obsfornn[1]
        a2 = obsfornn[2]
        b = np.zeros((self.vehicles_count, 3, 7))
        for i in range(self.vehicles_count):
            b[i][0] = a0[i]
            b[i][1] = a1[i]
            b[i][2] = a2[i]
        model = load_model("F:\python\workspace\B-GAP-H\\test\lstm\model\\intpre_num5_agg100%.h5")
        # print(b)
        pre = model.predict(b)

        return pre

    def observe(self, obsfornn_all: list) -> np.ndarray:
        if not self.env.road:
        ...
        # 判断观察空间是否达到神经网络需求 满足的话迭代删除多余数据
        obsfornn_now = df[self.intention_feature]
        # print(obsfornn_now.shape)
        obsfornn_all.append(obsfornn_now)
        obsfornn_all_ndarray = np.array(obsfornn_all)
        # print(obsfornn_all_ndarray[-1])

        if 'pre_x' and 'pre_y' in self.features:
            # 通过nn预测pre值并填入观察中
            if obsfornn_all_ndarray.shape[0] > 3:
                obsfornn_final = obsfornn_all_ndarray[-3:]
                np.save("test1.npy", obsfornn_final)
                # print(np.array(obsfornn_final).shape)
                # print(type(obsfornn_final))
                pre = self.pre_intention(obsfornn_final)
                df['pre_x'] = pre[:, 0]
                df['pre_y'] = pre[:, 1]
                # print(obsfornn_final[0])
            else:
                df['pre_x'] = 0
                df['pre_y'] = 0
            # Reorder
