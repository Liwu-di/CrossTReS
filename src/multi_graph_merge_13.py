...
if args.use_linked_region == 0:
    ...
    def calculate_linked_regions(t1, need_graph=False, threshold=0.2, city = 0):
        mask_t1 = t1 > threshold
        if need_graph:
            import seaborn as sns
            fig = sns.heatmap(mask_t1)
        ...
        for i in range(mask_t1.shape[0]):
            for j in range(mask_t1.shape[1]):
                if mask_t1[i][j]:
                    boxes.append([i, j, city])
        return boxes, mask_t1


    box1, linked_regions_range1 = calculate_linked_regions(s1_time_weight, False, args.s1_rate, 0)
    box2, linked_regions_range2 = calculate_linked_regions(s2_time_weight, False, args.s2_rate, 1)
    box3, linked_regions_range3 = [], []
    if args.need_third == 1:
        box3, linked_regions_range3 = calculate_linked_regions(s3_time_weight, False, args.s3_rate, 2)
    ...
    def find_m(n):
        ...
        while True:
            closest_a_b = None
            min_diff = float('inf')  # 记录 a 和 b 差值的最小值
            for a in range(4, int(math.sqrt(m)) + 1):  # a 从 4 开始，因为要求 a > 3
                if m % a == 0:
                    b = m // a
                    if b > 3:
                        diff = abs(a - b)  # 计算 a 和 b 的差值
                        if diff < min_diff:  # 找到 a 和 b 差值更小的组合
                            closest_a_b = (a, b)
                            min_diff = diff
            if closest_a_b:  # 如果找到了满足条件的 a 和 b
                return m, closest_a_b[0], closest_a_b[1]
            m += 1  # 如果没有找到，增加 m 再试
    sums = sum([len(box1), len(box2), len(box3)])
    boxs = []
    boxs.extend(box1)
    boxs.extend(box2)
    boxs.extend(box3)
    m, a, b = find_m(sums)
    virtual_source_coord = np.zeros((a, b, 3)) # length weight (source_x, source_y, city)
    virtual_city = np.zeros((args.cut_data, a, b))
    virtual_poi = np.zeros((a, b, 14))
    virtual_road = np.zeros((m, m))
    virtual_od = np.zeros((m, m))
    count = 0
    for i in range(a):
        for j in range(b):
            if i * b + j >= sums - 1:
                virtual_source_coord[i][j] = [-1, -1, -1]
                continue
            virtual_source_coord[i][j] = boxs[count]
            count = count + 1
    for m in range(virtual_source_coord.shape[0]):
        for n in range(virtual_source_coord.shape[1]):
            if virtual_source_coord[m][n][2] == 0:
                city = source_data
                poi = source_poi
                shape = (source_data.shape[1], source_data.shape[2])
                poi = poi.reshape((shape[0], shape[1], 14))
            elif virtual_source_coord[m][n][2] == 1:
                city = source_data2
                poi = source_poi2
                shape = (source_data2.shape[1], source_data2.shape[2])
                poi = poi.reshape((shape[0], shape[1], 14))
                ...
                city = source_data
            if virtual_source_coord[m][n][2] != -1:
                for i in range(args.cut_data):
                    virtual_city[i][m][n] = city[i][int(virtual_source_coord[m][n][0])][int(virtual_source_coord[m][n][1])]
                virtual_poi[m][n] = poi[int(virtual_source_coord[m][n][0])][int(virtual_source_coord[m][n][1])]
    for i in range(virtual_source_coord.shape[0] * virtual_source_coord.shape[1]):
        for j in range(virtual_source_coord.shape[0] * virtual_source_coord.shape[1]):
            m, n = idx_1d22d(i, (virtual_source_coord.shape[0], virtual_source_coord.shape[1]))
            p, q = idx_1d22d(j, (virtual_source_coord.shape[0], virtual_source_coord.shape[1]))
