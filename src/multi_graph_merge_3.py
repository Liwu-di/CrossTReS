...
def meta_train_epoch(s_embs, s2_embs, t_embs):
    ...
    for meta_ep in range(args.outeriter):
        ...
        for meta_it in range(args.sinneriter):
            s_x1, s_y1 = batch_sampler((torch.Tensor(source_train_x), torch.Tensor(source_train_y)), int(args.batch_size) / 2)
            s_x2, s_y2 = batch_sampler((torch.Tensor(source_train_x2), torch.Tensor(source_train_y2)), int(args.batch_size) / 2)
            s_x1 = s_x1.to(device)
            s_x2 = s_x2.to(device)
            s_y1 = s_y1.to(device)
            s_y2 = s_y2.to(device)
