# -*- coding: utf-8 -*-
# @Time    : 2022/12/16 16:26
# @Author  : 银尘
# @FileName: multi_graph_merge_4.py
# @Software: PyCharm
# @Email   ：liwudi@liwudi.fun
# @info    ：尝试分区域，从几个区域学习知识，然后预测一个区域，循环这个过程
import argparse
import ast
import time
...
if args.scoring == 1:
    ...
    class Scoring(nn.Module):
        ...
        def single_forward(self, source_emb, target_emb):
            target_context = torch.tanh(self.score(target_emb))
            source_trans_emb = self.score(source_emb)
            source_score = (source_trans_emb * target_context).sum(1)
...
def get_eight(id, length, height):
    res = []
    for p in [-1, 0, 1]:
        for q in [-1, 0, 1]:
            if id[0] + p < length and id[1] + q < height:
                res.append((id[0] + p, id[1] + q))
            else:
                res.append((-1, -1))
    return res
...
def get_eight_by_id(ids, length, weight, data, return_concat=True):
    ...
    for i in ids:
        eight = get_eight(i, length, weight)
        ...
        for p in [0, 1, 2]:
            for q in [0, 1 ,2]:
                if eight[count][0] == -1 and eight[count][1] == -1:
                    ...
                    tem[:, p, q] = data[:, eight[count][0], eight[count][1]]
                count = count + 1
        tensor_list.append(tem)
    if return_concat:
        return torch.concat(tensor_list, dim=2)
    else:
        return tensor_list


"""
以下代码生成420=21*20个data_loader,分区域预测
"""
...
for ep in range(num_epochs, num_tuine_epochs + num_epochs):
    ...
    """
    分区域进行预测，思路：
    0. 之前预训练是普适性（对于目标城市来说）的学习，对于目标城市整体有帮助，但是对于某个区域，不一定就是最好的
    1. 分区域，假设我们已知了最佳的weight，也是对于整体的，但是scoring网络是训练好的，可以对目标城市某个区域Rt，与源城市对比，
    求出几个最佳的区域，
    2. 进行再次预训练，使得模型更加适合预测Rt
